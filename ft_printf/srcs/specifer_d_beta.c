...
/*   Updated: 2020/03/02 09:28:58 by jwilen           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		check_d_epsilion(t_tab *dt, intmax_t nbr, int len, char *str)
{
	...
	(dt->wi > ft_strlen(str)) ? insert_pad(dt, len + 1) : 0;
	(dt->wi < ft_strlen(str)) ? insert_pad_zero(dt, len) : 0;
	write(dt->fd, str, len);
}
...
{
	(nbr >= 0) ? write(dt->fd, " ", 1) : 0;
	if (dt->pre_i.pre >= ft_strlen(str) && nbr >= 0 && !dt->w_i.mi)
		insert_pad(dt, dt->pre_i.pre);
	if (dt->pre_i.pre < ft_strlen(str) && nbr >= 0 && !dt->w_i.mi)
		insert_pad(dt, dt->pre_i.pre + 1);
	if (nbr < 0 && dt->pre_i.pre >= ft_strlen(str) && !dt->w_i.mi)
		insert_pad(dt, dt->pre_i.pre + 1);
	if (nbr < 0 && dt->pre_i.pre < ft_strlen(str) && !dt->w_i.mi)
		insert_pad(dt, len + 1);
	(nbr < 0) ? write(dt->fd, "-", 1) : 0;
	...
	write(dt->fd, str, len);
	if ((dt->pre_i.pre > ft_strlen(str)) && dt->w_i.mi && nbr < 0)
		insert_pad(dt, dt->pre_i.pre + 1);
	if ((dt->pre_i.pre < ft_strlen(str)) && dt->w_i.mi && nbr < 0)
		insert_pad(dt, len + 1);
	if ((dt->pre_i.pre > ft_strlen(str)) && dt->w_i.mi && nbr >= 0)
...
{
	if (dt->w_i.sp && nbr >= 0 && (!dt->w_i.pl) && (dt->wi <= ft_strlen(str)))
		write(dt->fd, " ", 1);
	if (dt->w_i.sp && nbr >= 0 && (!dt->w_i.pl) && (dt->wi > ft_strlen(str)))
		if (dt->w_i.mi)
			write(dt->fd, " ", 1);
